...
export default function BuyForm() {
  ...
  async function onSubmit(values) {
    try {
      ...
      const payload = {
        plan: values.plan,
        imeIPrezime: values.imeIPrezime,
        telefon: values.telefon,
        email: values.email,
        adresa: values.adresa,
        _replyto: values.email,
        _subject: 'Nova porudzbina sa sajta'
      }

      const res = await fetch('https://formspree.io/f/mjgklqvv', {
        method: 'POST',
        headers: {
      ...
      // read raw response for debugging (Formspree sometimes returns text)
      let raw = null
      try {
        raw = await res.text()
      } catch (e) {
        raw = `could not read response body: ${e.message}`
      }
      console.log('Formspree response status:', res.status, 'body:', raw)

      if (res.ok) {
        try {
          const json = JSON.parse(raw || '{}')
          console.log('Formspree json:', json)
        } catch (e) {
          // ignore parse error
        }
        form.reset()
      ...
      throw new Error(`Formspree returned ${res.status}: ${raw}`)
    } catch (err) {
      setError(err.message || 'Doslo je do greske prilikom slanja')
    } finally {
      setLoading(false)
    }
  }
